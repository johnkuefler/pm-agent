// Memory
    ...
    let _memoryStats = null;

    async function loadMemory() {
      const list = document.getElementById('memory-list');
      try {
        const [memRes, projRes, statsRes, digestRes] = await Promise.all([
          api('/memory'), api('/projects'), api('/memory?view=stats'), api('/memory?view=digest')
        ]);
        const memories = await memRes.json();
        const projects = await projRes.json();
        _memoryStats = await statsRes.json();
        ...
        _allMemories = memories.map((m, i) => ({ ...m, _idx: i, _tier: memoryTier(m) }));
        _memoryRenderCap = 80;
    ...
    function memoryTier(memory) {
      if (memory.memory_tier) return memory.memory_tier;
      if (memory.status && memory.status !== 'active') return 'archive';
      if (memory.valid_until && new Date(memory.valid_until).getTime() < Date.now()) return 'archive';
      const observed = memory.valid_from || (memory.source_ref && memory.source_ref.captured_at) || memory.added;
      const timestamp = observed ? new Date(observed).getTime() : NaN;
      return Number.isFinite(timestamp) && Date.now() - timestamp > 30 * 86400000
        ? 'long_term' : 'working';
    }

    function renderMemoryDigest(digest) {
      const el = document.getElementById('memory-digest');
      if (!el) return;
      const counts = _memoryStats
        ? `Working ${_memoryStats.working || 0} | Long term ${_memoryStats.long_term || 0} | Archive ${_memoryStats.archive || 0}\n\n`
        : '';
      el.textContent = counts + (digest && digest.text
        ? digest.text
        : 'The daily digest will be generated by the next memory maintenance run.');
    }
    ...
    function renderMemory() {
      ...
      const tierFilter = document.getElementById('memory-tier').value;
      ...
      if (tierFilter) items = items.filter(m => m._tier === tierFilter);
      ...
      countEl.textContent = q || sourceFilter || tierFilter
    ...
    function memoryItemHtml(m) {
      ...
      return `<div class="memory-item" id="memory-${escHtml(String(key))}" data-memory-key="${escHtml(String(key))}">
        <div style="flex: 1;">
          ...
          <div class="memory-meta">${m.added || ''}${m.source ? ' · ' + m.source : ''}${m.project ? ' · ' + escHtml(m.project) : ''}${m.kind ? ' · ' + escHtml(m.kind) : ''}${m.retention_class ? ' · ' + escHtml(m.retention_class) : ''}${m._tier ? ' · ' + escHtml(m._tier) : ''}${m.confidence != null ? ' · ' + Math.round(m.confidence * 100) + '% confidence' : ''}${m.status && m.status !== 'active' ? ' · ' + escHtml(m.status) : ''}${m.last_verified ? ' · verified ' + new Date(m.last_verified).toLocaleDateString() : ''}${transcriptLink}</div>
        </div>
    ...
    async function addMemory() {
      ...
      const retention_class = document.getElementById('new-fact-retention').value;
      ...
      try {
        await api('/memory', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fact, project, kind, retention_class, confidence, last_verified: new Date().toISOString() }) });
