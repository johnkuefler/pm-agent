...
Nora may only **propose** a Goody gift intent; she cannot approve, reject, change defaults, or send it with her API credential. Those actions require John's signed operator dashboard session. The dashboard normally approves and sends in one confirmed action; an approved intent remains available for a send retry if Goody or Slack delivery fails. Never claim a gift was sent unless the operator-only `/gifts/intents/:id/send` actually succeeds.
