...
test('runtime enables reappraisal only in background-capable production mode', () => {
  ...
  assert.match(source,
    /scheduleRecurringRuntimeJob\('background-intelligence-cycle'[\s\S]*?runBackgroundIntelligenceRuntime/,
    'all scheduled intelligence work should enter one non-overlapping recurring owner');
  assert.match(source, /scheduleRecurringRuntimeJob\('background-intelligence-cycle'[\s\S]*?initialDelayMs: 2 \* 60 \* 1000/,
    'the intelligence owner should provide a delayed startup run and completion-aware cadence');
